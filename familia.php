...
<? 
$noticia = mysql_query("SELECT * FROM familia_master");
$notician = mysql_num_rows($noticia);
if (!$noticia){
?>Problemas na conexão<?
}
...
<?
$sql = mysql_query ("INSERT INTO familia (nome,grupo,master) VALUES ('$nome','$grupo','$master');");
if (!$sql){
...
<?
$noticia = mysql_query("SELECT * FROM familia where id = '$id' order by nome");
$notician = mysql_num_rows($noticia);
if (!$noticia){
?>Problemas na conexão<?
}
...
<select size="1" name="master" class=form-especial>
<? 
$anoticia = mysql_query("SELECT * FROM familia_master");
...
$sql = mysql_query("UPDATE familia SET nome='$nome', grupo='$grupo', master='$master' WHERE id='$id' LIMIT 1;");
if (!$sql){
echo "Não foi possivel a consulta.";}
else{
?>
