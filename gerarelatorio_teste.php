...
if ($notician==0){
?><? } else { ?><? while ($dom = mysql_fetch_array($noticia)){ ?>
<? $total = $dom[obra] + $dom[disponivel] + $dom[manutencao]; ?>
...
<? } ?>    
  ...
  <tr>
    <td><font face="Verdana" size="2">&nbsp;Horas Vendidas</font></td>
<?  for($i=1; $i< $num; $i++) { ?>
<td>
<?
...
<? } ?>    
    <td><center><? echo number_format($fim_pont * 9,2,',','.'); ?></td>
  </tr>
  <tr>
    <td><font face="Verdana" size="2">&nbsp;Pontos Marcados</font></td>
<?  for($i=1; $i< $num; $i++) { ?>
    <td><center>
    <? 
$noticia = mysql_query("SELECT * FROM dip_diaria_geral where mes = $mes And ano = $ano And data =  $i");
...
<? } ?>    
    <td><center><? echo number_format($cami,2,',','.'); ?></center></td>
  </tr>
  <tr>
