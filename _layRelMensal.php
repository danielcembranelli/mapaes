...
<body>
<center>
<h2>RELATÓRIO MENSAL DE ENTRADA (<?=$dat?>)</h2>
  ...
  <tr>
    ...
    <td width="15%" bgcolor="#FF0000"><span class="style1">Motivo</span></td>
    <td width="15%" bgcolor="#FF0000"><div align="center" class="style1">Status Final</div></td>
  </tr>
  <?
	$SqlObra = mysql_query("SELECT c.contrato, c.cliente, f.nome as NOMEFAMILIA,q.status,e.motivo_final, e.obra,q.CODIGO as eqtoCodigo ,e.operador, o.nome as operadorNome, e.acessorio, a.CODIGO as acessorioCodigo, DATE_FORMAT(e.inicio,'%d/%m/%Y') as dtInicio, DATE_FORMAT(e.fim,'%d/%m/%Y') as dtFim, e.horimetro_ida, e.horimetro_volta FROM equipamento_obra e inner join equipamento q on (q.id=e.equipamento) left join familia f on (q.familia=f.id) left join acessorio a on (a.id=e.acessorio) left join operador o on (o.id=e.operador) left join obra c on (c.id=e.obra) where DATE_FORMAT(e.dtvoltaRel,'%Y-%m') = '".$_REQUEST[dt]."' order by dtvoltaRel;") or die (mysql_error());
	$i=0;
...
?>
  ...
  <td><div align="center"><?=$sE[cliente]?></div></td>
    ...
    <td align="center"><?=$sE[motivo_final]?></td>
...
<h2>RELATÓRIO MENSAL DE SAIDAS (<?=$dat?>)</h2>
  <table width="800" border="1" bordercolor="#000000" cellpadding="5" cellspacing="0">
  <tr>
  ...
  <?
	$SqlObra = mysql_query("SELECT c.contrato, c.cliente, f.nome as NOMEFAMILIA, e.obra,q.CODIGO as eqtoCodigo ,e.operador, o.nome as operadorNome, e.acessorio, a.CODIGO as acessorioCodigo, DATE_FORMAT(e.inicio,'%d/%m/%Y') as dtInicio, DATE_FORMAT(e.fim,'%d/%m/%Y') as dtFim, e.horimetro_ida, e.horimetro_volta FROM equipamento_obra e inner join equipamento q on (q.id=e.equipamento) left join familia f on (q.familia=f.id) left join acessorio a on (a.id=e.acessorio) left join operador o on (o.id=e.operador) left join obra c on (c.id=e.obra) where DATE_FORMAT(e.dtinicioRel,'%Y-%m') = '".$_REQUEST[dt]."' order by dtinicioRel;");
	$i=0;
	while ($sE = mysql_fetch_array($SqlObra)){
		$i++;
	$cor = ($coralternada++ %2 ? "FFFFFF" : "FFFFDC");
